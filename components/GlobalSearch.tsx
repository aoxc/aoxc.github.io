...
interface SearchResult {
  ...
  title: string
  subtitle: string
  type: 'MODULE' | 'PROPOSAL' | 'USER' | 'SYSTEM'
  page: PageView
  status?: string
}
...
export const GlobalSearch: React.FC<GlobalSearchProps> = ({ setPage }) => {
  ...
  useEffect(() => {
    ...
    // Search Telemetry
    if (
      'telemetry'.includes(lowerQuery) ||
      'monitor'.includes(lowerQuery) ||
      'network'.includes(lowerQuery) ||
      'logs'.includes(lowerQuery)
    ) {
      newResults.push({
        id: 'telemetry-page',
        title: 'Network Telemetry',
        subtitle: 'Live System Monitoring & Logs',
        type: 'SYSTEM',
        page: PageView.TELEMETRY,
        status: 'Live'
      })
    }
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case 'PROPOSAL':
        return <FileText className="w-4 h-4 text-xlayer-magenta" />
      case 'USER':
        return <User className="w-4 h-4 text-xlayer-green" />
      case 'SYSTEM':
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      ...
      case 'PROPOSAL':
        return 'bg-xlayer-magenta/10 text-xlayer-magenta border-xlayer-magenta/20'
      case 'USER':
        return 'bg-xlayer-green/10 text-xlayer-green border-xlayer-green/20'
      case 'SYSTEM':
  ...
  return (
    <div className="relative w-full max-w-2xl mx-auto z-50" ref={containerRef}>
      ...
      <div className={`relative group transition-all duration-300 ${isOpen ? 'scale-105' : ''}`}>
        <div
          className={`absolute -inset-0.5 bg-gradient-to-r from-xlayer-green via-cyan-500 to-xlayer-magenta rounded-xl opacity-20 group-hover:opacity-60 blur transition duration-500 ${isOpen ? 'opacity-80' : ''}`}
        ></div>
                ...
                >
                  ...
                  <div className="text-left flex-1">
                    <div className="text-white font-medium text-sm group-hover:text-xlayer-green transition-colors flex items-center gap-2">
                      ...
                      {result.status && (
                        <span
                          className={`text-[9px] px-1.5 py-0.5 rounded border ${result.status === 'Active' || result.status === 'Live' ? 'border-green-500/30 text-green-400' : 'border-gray-500/30 text-gray-400'}`}
                        >
                          {result.status}
                        </span>
                      )}
            ...
            <div className="p-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className="flex items-center gap-3 p-3 rounded-lg bg-white/5 hover:bg-white/10 border border-white/5 hover:border-xlayer-green/30 transition-all text-left"
                >
                  <div className="w-8 h-8 rounded-full bg-xlayer-green/20 flex items-center justify-center text-xlayer-green">
                ...
                <button
                  ...
                  className="flex items-center gap-3 p-3 rounded-lg bg-white/5 hover:bg-white/10 border border-white/5 hover:border-yellow-500/30 transition-all text-left"
                >
                  <div className="w-8 h-8 rounded-full bg-yellow-500/20 flex items-center justify-center text-yellow-500">
                    <Activity size={16} />
                  </div>
                  <div>
                    <div className="text-xs font-bold text-white">Telemetri</div>
                    <div className="text-[10px] text-gray-500">Ağ Durumu</div>
                  </div>
                </button>
              </div>
            </div>
