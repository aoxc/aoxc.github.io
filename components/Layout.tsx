...
const GlassSidebarItem = ({ 
  ...
  >
    ...
    <Icon className={`w-5 h-5 flex-shrink-0 transition-colors z-10 ${active ? 'text-xlayer-green drop-shadow-[0_0_8px_rgba(0,255,163,0.6)]' : 'text-gray-500 group-hover:text-gray-300'}`} />
    
    {/* Label transition */}
    <span className={`z-10 font-sans tracking-wide font-medium text-sm whitespace-nowrap overflow-hidden transition-all duration-300 ${collapsed ? 'w-0 opacity-0' : 'w-auto opacity-100'}`}>
      {label}
    </span>
...
export const Layout: React.FC<LayoutProps> = ({ children, currentPage, setPage }) => {
  ...
  return (
    <div className="min-h-screen w-full bg-[#000000] text-gray-100 flex overflow-hidden relative selection:bg-xlayer-green selection:text-black">
      ...
      >
        {/* Sidebar Header / Logo */}
        <div className={`p-4 flex flex-col justify-center border-b border-white/5 h-24 transition-all duration-300 ${isCollapsed ? 'items-center' : 'items-start'}`}>
          <div className="flex items-center gap-2">
             <AOXCLogo className={`transition-all duration-300 ${isCollapsed ? 'h-10 w-10' : 'h-20 w-auto'}`} collapsed={isCollapsed} />
          </div>
        </div>
        ...
        >
          {isCollapsed ? <ChevronRight size={14} className="group-hover:text-xlayer-green" /> : <ChevronLeft size={14} className="group-hover:text-xlayer-green" />}
        </button>

        {/* Navigation */}
        <nav className="flex-1 p-3 overflow-y-auto overflow-x-hidden scrollbar-thin">
          <div className="mb-6">
            {!isCollapsed && <p className="px-4 text-[10px] font-bold text-gray-600 uppercase tracking-widest mb-3 opacity-70">{t('nav_main_deck')}</p>}
            <GlassSidebarItem 
          ...
          <div className="mb-6">
            {!isCollapsed && <p className="px-4 text-[10px] font-bold text-gray-600 uppercase tracking-widest mb-3 opacity-70">{t('nav_engineering')}</p>}
             <GlassSidebarItem 
              icon={Code2} 
          ...
          <div className="mb-6">
            {!isCollapsed && <p className="px-4 text-[10px] font-bold text-gray-600 uppercase tracking-widest mb-3 opacity-70">{t('nav_finance')}</p>}
            <GlassSidebarItem 
              icon={ArrowRightLeft} 
              label={t('nav_swap')}
           ...
           <div>
            {!isCollapsed && <p className="px-4 text-[10px] font-bold text-gray-600 uppercase tracking-widest mb-3 opacity-70">{t('nav_identity')}</p>}
            <GlassSidebarItem 
              icon={UserCircle} 
              label={t('nav_profile')} 
        ...
        <div className="p-4 border-t border-white/5 bg-black/40 overflow-hidden backdrop-blur-sm">
          <div className={`flex items-center transition-all duration-300 ${isCollapsed ? 'justify-center flex-col gap-2' : 'justify-between'}`}>
            <div className="flex items-center gap-2" title="Network Status: OKX X Layer">
              ...
              {!isCollapsed && <span className="text-xs text-white font-medium tracking-wide">{network ? network.name : '...'}</span>}
            </div>
            {!isCollapsed && <span className="text-[10px] text-gray-500 font-mono">#{network?.block}</span>}
          </div>
        </div>
      </aside>
      ...
      <div className="lg:hidden fixed top-0 w-full h-16 bg-black/95 backdrop-blur-xl border-b border-white/10 flex items-center justify-between px-4 z-50 shadow-lg shadow-black/50">
        ...
        <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="text-white p-2 active:scale-95 transition-transform">
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden fixed inset-0 z-40 bg-black pt-20 px-4 pb-10 overflow-y-auto">
          <div className="flex flex-col gap-1">
            <GlassSidebarItem collapsed={false} icon={LayoutDashboard} label={t('nav_dashboard')} active={currentPage === PageView.DASHBOARD} onClick={() => { setPage(PageView.DASHBOARD); setIsMobileMenuOpen(false); }} />
            <GlassSidebarItem collapsed={false} icon={Code2} label={t('nav_contracts')} active={currentPage === PageView.CONTRACTS} onClick={() => { setPage(PageView.CONTRACTS); setIsMobileMenuOpen(false); }} />
            <GlassSidebarItem collapsed={false} icon={Vote} label={t('nav_governance')} active={currentPage === PageView.GOVERNANCE} onClick={() => { setPage(PageView.GOVERNANCE); setIsMobileMenuOpen(false); }} />
            <GlassSidebarItem collapsed={false} icon={ArrowRightLeft} label={t('nav_swap')} active={currentPage === PageView.SWAP} onClick={() => { setPage(PageView.SWAP); setIsMobileMenuOpen(false); }} />
            <GlassSidebarItem collapsed={false} icon={UserCircle} label={t('nav_profile')} active={currentPage === PageView.PROFILE} onClick={() => { setPage(PageView.PROFILE); setIsMobileMenuOpen(false); }} />
          </div>
          ...
          <div className="flex items-center gap-4 pl-8">
             ...
             <div className="hidden xl:flex items-center gap-4 px-4 py-2 rounded-full border border-white/10 bg-[#0A0A0A] mr-4 shadow-inner shadow-black/50">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-400">Gas: <span className="text-white font-mono">0.001 OKB</span></span>
                </div>
                <div className="w-px h-3 bg-white/20"></div>
                <div className="flex items-center gap-2">
                  <span className="text-xs text-gray-400">AOXC: <span className="text-xlayer-green font-mono font-bold">$1.24</span></span>
                </div>
             </div>
            ...
            {user ? (
              <div className="flex items-center gap-3 bg-[#0A0A0A] border border-white/10 px-4 py-2 rounded-full hover:border-xlayer-green/50 transition-colors cursor-pointer group shadow-lg">
                <div className="text-right hidden md:block">
                  ...
                  <div className="text-[10px] text-gray-400 tracking-wider font-medium group-hover:text-xlayer-green transition-colors">{user.role}</div>
                </div>
