...
/* ===============================
   CONFIG
================================ */
// Use a public RPC or fallback to simulation if CORS fails
...
const generateMockData = (): ChainData => {
  ...
  const mockEvents: ChainEvent[] = Array.from({ length: 5 })
    .map((_, i) => ({
...
export const fetchChainIntel = async (): Promise<ChainData> => {
  try {
    // Use staticNetwork to prevent immediate network probing that causes fetch errors
    const provider = new ethers.JsonRpcProvider(RPC_URL, undefined, { staticNetwork: true })

    // Fail fast on RPC issues
    ...
    const fetchData = async (): Promise<ChainData> => {
      /* ===== BLOCK INTEL ===== */
      // This is the first network call. If it fails (CORS, Offline), we catch and mock.
      ...
      /* ===== EVENT FEED ===== */
      const fromBlock = blockNumber - 20 // Keep range small to avoid timeouts
      let logs: ChainEvent[] = []
      ...
      for (const addr of AOXC_CONTRACTS) {
        try {
          ...
          // Ignore logs if they fail
        }
      }

      const sortedEvents = logs.sort((a, b) => b.block - a.block).slice(0, 10)
